...
You can directly use [HTML formatting (for HTML output files) and PDF formatting (for PDF output files)](https://quarto.org/docs/authoring/markdown-basics.html#raw-content).
...
The macros for HTML rendering should be placed in a .md file, formatted as shown in [_macros.md](_macros.md).
...
When using `knitr` engine, output appears directly after the code producing the output. 
...
But you can add `ipynb-shell-interactivity: all` to the yml header of the file to have all output printed.
...
The Quarto syntax for specifying a chunk label (useful for debugging) and chunk options is as shown here:
