...
You can embed Python code. As with R, state is preserved so later chunks can use objects from earlier chunks, if you have a sufficiently up-to-date version of the `rmarkdown` package (and of RStudio if you are working with Python chunks within RStudio). 
...
Note that on older versions of RStudio and with older versions of the `rmarkdown` package, the functionality for running Python chunks was much more limited:

 - as with bash chunks, all output occurred after all the code in a chunk was printed.
 - state was not preserved between chunks
 - one needed an explicit print statement to see the value of an object
