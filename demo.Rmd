...
```
```{r, eval, eval=FALSE}
cat("this code is not evaluated but is echoed to the screen")
```
...
In bash, we have similar problems with lines overflowing in PDF output, but bash allows us to use a backslash to break lines of code. The one thing that doesn't help with is long lines of output. 
