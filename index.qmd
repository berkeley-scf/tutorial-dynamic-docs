...
:::{.callout-tip}
As of February 2025, the website for this tutorial was developed using Quarto.

The reason that some of the files in the tutorial/repository start with "_" is to prevent Quarto from trying to render those files to HTML when building the website, so that the website can link to them as actual source files for you to examine.
:::
...
You can insert code chunks in a different language using the `%%` magic syntax, as shown in [_demo-python.ipynb](_demo-python.ipynb). We also have specific demo files for bash and R: [_demo-bash.ipynb](_demo-bash.ipynb) and [_demo-R.ipynb](_demo-R.ipynb).  All include instructions for generating HTML output. 
